...
const SortableRowWrapper: React.FC<SortableRowWrapperProps> = ({ id, isManualSort, children, className }) => {
      ...
      {React.Children.map(children, (child, index) => {
        if (index === 0 && React.isValidElement(child)) {
          return React.cloneElement(child, {
            // @ts-expect-error - overriding children
            children: (
              <div className="flex items-center gap-3">
                {isManualSort && (
                  <div {...attributes} {...listeners} className="cursor-grab active:cursor-grabbing text-slate-300 hover:text-indigo-500 touch-none">
                ...
                {rows.map((row) => {
                  return (
                    <SortableRowWrapper
                      ...
                      className={`hover:bg-slate-50 transition-colors ${row.isJoint ? "bg-purple-50/30" : ""}`}
                    >
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              {row.isJoint && (
                                <span className="text-[9px] bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded uppercase tracking-wide">PIVOT</span>
                              )}
                      ...
                      <td className="px-4 py-2.5 text-right bg-indigo-50/30 font-bold font-mono">
                        <div className="flex flex-col items-end gap-1">
                          ...
                          <span className="text-[9px] text-indigo-600 uppercase font-medium">{getTransferLabel(row)}</span>
                        </div>
                      </td>
                    </SortableRowWrapper>
                  );
