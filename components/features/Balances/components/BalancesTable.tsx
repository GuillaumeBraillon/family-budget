          ...
          <table className="w-full text-sm text-left">
              <thead className="bg-slate-50 text-slate-500 font-semibold border-b border-slate-200 uppercase tracking-wider text-xs">
                  <tr>
                      ...
                      <th className="px-4 py-2.5 text-right">
                          Solde prévu
                          <MobileTooltip text="Solde projeté après virement (Solde Actuel + Virement). Correspond à l'objectif à atteindre une fois le transfert effectué." iconClassName="text-indigo-300 hover:text-indigo-100 transition-colors" />
                      </th>
                      <th className="px-4 py-2.5 text-right bg-indigo-50/50 text-indigo-900">
                          Virement
                          <MobileTooltip text="Montant à ajouter pour atteindre la cible (arrondi à 5€). Si Négatif, c'est un excédent à virer vers le compte pivot." iconClassName="text-indigo-600 hover:text-indigo-400 transition-colors" />
                      </th>
                  </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                  {rows.map(row => {
                      ...
                      const roundedBalance = Math.round(row.balance); 
                      const roundedTransfer = roundTransfer(row.transfer);
                      // On recalcule le solde prévu affiché basé sur les valeurs arrondies
                      const displayedTarget = roundedBalance + roundedTransfer;
                      ...
                      <tr key={row.id} className={`hover:bg-slate-50 transition-colors ${row.isJoint ? 'bg-purple-50/30' : ''}`}>
                                  ...
                                  <div>
                                      ...
                                      {!row.isJoint && (row.ratio !== undefined || row.cap !== undefined) && (
                                          <div className="flex flex-wrap gap-1 mt-0.5 animate-in slide-in-from-left-2 duration-300">
                                              {row.ratio !== undefined && (
                                                  <span className="text-[9px] font-bold text-indigo-600 bg-indigo-50 px-1.5 py-0 rounded border border-indigo-100 flex items-center gap-1" title="Ratio appliqué sur le reste à vivre">
                                                      <Percent size={8} /> {row.ratio}%
                                                  </span>
                                              )}
                                              {row.cap !== undefined && (
                                                  <span className="text-[9px] font-bold text-slate-500 bg-slate-100 px-1.5 py-0 rounded border border-slate-200 flex items-center gap-1" title="Plafond maximum de virement">
                          ...
                          <td className="px-4 py-2.5 text-right">
                              <div className="inline-block px-1.5 py-0.5 bg-slate-100 rounded text-slate-700 font-mono font-bold text-xs" title={`Exact: ${row.target.toFixed(2)} €`}>
                                  {displayedTarget} €
                              </div>
                              {row.isJoint && (
                                  <p className="text-[9px] text-slate-400 mt-0.5">Inclus factures</p>
                              )}
                          </td>
                          <td className="px-4 py-2.5 text-right bg-indigo-50/30 font-bold font-mono">
                              <div className="flex flex-col items-end">
                                  <span className={`text-sm ${row.transfer > 0 ? 'text-indigo-600' : 'text-emerald-600'}`} title={`Exact: ${row.transfer > 0 ? '+' : ''}${row.transfer.toFixed(2)} €`}>
                                      {row.transfer > 0 ? '+' : ''}{roundedTransfer} €
                                  </span>
              ...
              {totalRow && (
                  <tfoot className="bg-slate-100 border-t-2 border-slate-200 font-bold text-slate-900 text-xs">
                      <tr>
                          ...
                          <td className="px-4 py-2.5 text-right">
                              {Math.round(totalRow.balance)} €
                          </td>
                          <td className="px-4 py-2.5 text-right">
                              {/* Cohérence aussi pour le total : Solde + Virement */}
                              {Math.round(totalRow.balance) + roundTransfer(totalRow.transfer)} €
                          </td>
                          <td className="px-4 py-2.5 text-right bg-indigo-100/50 text-indigo-800">
                              {roundTransfer(totalRow.transfer) > 0 ? '+' : ''}{roundTransfer(totalRow.transfer)} €
                          </td>
                      </tr>
                  </tfoot>
              )}
