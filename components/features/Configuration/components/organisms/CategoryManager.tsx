  ...
  return (
    <div className="space-y-4">
      <ConfirmModal
        ...
        message={
          <span>
            Voulez-vous vraiment supprimer la catégorie <strong>{deleteConfirm?.name}</strong> ?
          </span>
        }
      ...
      <div className="grid gap-2">
        {currentList.map((cat) => (
          <div
            ...
            className={`bg-white border rounded-lg overflow-hidden transition-all ${
              isIncome ? "border-emerald-100 shadow-sm hover:border-emerald-200" : "border-slate-200 shadow-sm hover:border-slate-300"
            }`}
          >
              ...
              <div className="flex items-center gap-2 flex-1">
                ...
                {editingCatId === cat.id ? (
                  <div className="flex items-center gap-2 animate-in fade-in zoom-in-95 duration-200">
                    ...
                    />
                    <button onClick={() => saveCatName(cat.id)} className="text-green-600 hover:bg-green-50 p-1 rounded">
                      <Save size={16} />
                    </button>
                    <button onClick={() => setEditingCatId(null)} className="text-slate-400 p-1 rounded">
                      <X size={16} />
                    </button>
                  </div>
                ) : (
                    ...
                    <div key={idx} className="flex items-center justify-between text-sm pl-8 pr-2 py-1.5 hover:bg-slate-50 rounded group">
                      {editingSubCat?.catId === cat.id && editingSubCat?.oldName === sub ? (
                        <div className="flex items-center gap-2 flex-1 animate-in fade-in duration-200">
                          ...
                          />
                          <button onClick={saveSubCat} className="text-green-600">
                            <Save size={14} />
                          </button>
                          <button onClick={() => setEditingSubCat(null)} className="text-slate-400">
                            <X size={14} />
                          </button>
                        </div>
                      ) : (
        ...
        {currentList.length === 0 && <div className="p-8 text-center text-slate-400 italic text-sm">Aucune catégorie définie.</div>}
      </div>
    </div>
  );
};
