          ...
          <div>
            ...
            <div className="flex gap-3 text-xs font-medium text-indigo-200 mt-0.5">
              <span className="flex items-center gap-1">
                <CreditCard size={10} /> Dispo: {checkingTotal.toFixed(0)}€
              </span>
              <span className="w-px h-3 bg-indigo-400/30"></span>
              <span className="flex items-center gap-1">
                <PiggyBank size={10} /> Épargne: {savingsTotal.toFixed(0)}€
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {sortedAccounts.map((acc) => {
            ...
            return (
              <div
                key={acc.id}
                className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between group hover:border-indigo-200 transition-colors"
              >
                <div className="flex items-center gap-3 overflow-hidden">
                  <div
                    className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform ${isSavings ? "bg-emerald-50 text-emerald-600" : "bg-indigo-50 text-indigo-600"}`}
                  >
