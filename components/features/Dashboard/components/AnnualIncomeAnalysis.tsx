...
export const AnnualIncomeAnalysis: React.FC<AnnualIncomeAnalysisProps> = ({ data, year, onYearChange, onNavigateToPlanner }) => {
    ...
    const handleAmountClick = (e: React.MouseEvent, monthDate: Date, periodId: number | undefined, 
                               flux: 'EXPENSE' | 'INCOME', source: 'RECURRING' | 'VARIABLE' | 'ALL') => {
        e.stopPropagation();
        onNavigateToPlanner(
            monthDate, 
            {
                            ...
                            return (
                                <React.Fragment key={month.monthIndex}>
                                    ...
                                    <tr className="group hover:bg-emerald-50/20 transition-colors border-t-2 border-slate-100">
                                        ...
                                        {periodsHeader.map((p, idx) => (
                                            <td key={idx} className="px-3 py-1.5 text-right text-slate-500">
                                                {renderCell(month.periods[idx]?.income.recurring, month.dateObj, month.periods[idx]?.period.id, 'INCOME', 'RECURRING', handleAmountClick, 'text-emerald-600')}
                                            </td>
                                        ))}
                                    ...
                                    <tr className="group hover:bg-emerald-50/20 transition-colors">
                                        ...
                                        {periodsHeader.map((p, idx) => (
                                            <td key={idx} className="px-3 py-1.5 text-right text-slate-500">
                                                {renderCell(month.periods[idx]?.income.variable, month.dateObj, month.periods[idx]?.period.id, 'INCOME', 'VARIABLE', handleAmountClick, 'text-emerald-600')}
                                            </td>
                                        ))}
                                    ...
                                    <tr className="group hover:bg-rose-50/10 transition-colors">
                                        ...
                                        {periodsHeader.map((p, idx) => (
                                            <td key={idx} className="px-3 py-1.5 text-right text-slate-500">
                                                {renderCell(month.periods[idx]?.expenses.recurring, month.dateObj, month.periods[idx]?.period.id, 'EXPENSE', 'RECURRING', handleAmountClick, 'text-slate-600')}
                                            </td>
                                        ))}
                                    ...
                                    <tr className="group hover:bg-rose-50/10 transition-colors">
                                        ...
                                        {periodsHeader.map((p, idx) => (
                                            <td key={idx} className="px-3 py-1.5 text-right text-slate-500">
                                                {renderCell(month.periods[idx]?.expenses.variable, month.dateObj, month.periods[idx]?.period.id, 'EXPENSE', 'VARIABLE', handleAmountClick, 'text-indigo-600')}
                                            </td>
                                        ))}
...
const renderCell = (
    ...
    return (
        <button 
            onClick={(e) => onClick(e, date, periodId, flux, source)}
            className={`hover:underline transition-colors ${colorClass || ''}`}
        >
            {value.toFixed(2)} €
        </button>
    );
