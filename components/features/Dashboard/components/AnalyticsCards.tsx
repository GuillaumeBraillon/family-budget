        ...
        <Card className="flex flex-col h-full border-slate-200 shadow-sm hover:border-indigo-200 transition-colors">
            <CardHeader className="pb-2">
                <CardTitle className="flex justify-between items-center text-sm font-bold text-slate-700">
                    <span className="flex items-center gap-2"><Wallet size={16} className="text-indigo-600" /> Santé du Budget</span>
                    <button onClick={() => onNavigate('balances')} className="text-indigo-600 hover:text-indigo-800"><ArrowUpRight size={16}/></button>
                </CardTitle>
            </CardHeader>
            <CardContent className="flex-1 flex flex-col justify-between pt-0">
                <div className="mt-4">
                    ...
                    <div className={`p-2 rounded-lg text-xs font-bold flex justify-between items-center ${isOver ? 'bg-rose-50 text-rose-700' : 'bg-emerald-50 text-emerald-700'}`}>
                        ...
                        <span>{isOver ? '+' : ''}{formatEuro(Math.abs(remaining))}</span>
                    </div>
                </div>
...
export const CashFlowCard: React.FC<AnalyticsProps> = ({ data, onNavigate }) => {
    ...
    return (
        <Card className="flex flex-col h-full border-slate-200 shadow-sm hover:border-emerald-200 transition-colors">
            <CardHeader className="pb-2">
                <CardTitle className="flex justify-between items-center text-sm font-bold text-slate-700">
                    <span className="flex items-center gap-2"><ArrowUpRight size={16} className="text-emerald-600" /> Flux de Trésorerie</span>
                    <button onClick={() => onNavigate('planner')} className="text-emerald-600 hover:text-emerald-800"><ArrowUpRight size={16}/></button>
                </CardTitle>
            </CardHeader>
            <CardContent className="flex-1 pt-0">
                ...
                <div className="space-y-2">
                    <div className="flex justify-between items-center text-xs">
                        <span className="text-slate-500 font-medium flex items-center gap-1"><TrendingUp size={12} className="text-emerald-500"/> Entrées</span>
                        <span className="font-bold text-slate-800">{formatEuro(data.income)}</span>
                    </div>
                    <div className="flex justify-between items-center text-xs">
                        <span className="text-slate-500 font-medium flex items-center gap-1"><TrendingDown size={12} className="text-rose-500"/> Sorties</span>
                        <span className="font-bold text-slate-800">{formatEuro(data.expenses)}</span>
                    </div>
                    <div className={`mt-3 p-2 rounded-lg flex justify-between items-center text-sm font-bold ${net >= 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}>
                        <span>Reste à vivre</span>
                        <span>{net > 0 ? '+' : ''}{formatEuro(net)}</span>
                    </div>
                </div>
            </CardContent>
        </Card>
...
export const ExtrasCard: React.FC<AnalyticsProps> = ({ data, onNavigate }) => {
    ...
    return (
        <Card className="flex flex-col h-full border-slate-200 shadow-sm hover:border-amber-200 transition-colors">
            <CardHeader className="pb-2">
                <CardTitle className="flex justify-between items-center text-sm font-bold text-slate-700">
                    <span className="flex items-center gap-2"><Star size={16} className="text-amber-500" /> Analyse Extras</span>
                    <button onClick={() => onNavigate('planner')} className="text-amber-600 hover:text-amber-800"><ArrowUpRight size={16}/></button>
                </CardTitle>
            </CardHeader>
...
export const TopExpensesCard: React.FC<AnalyticsProps> = ({ data, onNavigate }) => {
    ...
    return (
        <Card className="flex flex-col h-full border-slate-200 shadow-sm hover:border-indigo-200 transition-colors">
            <CardHeader className="pb-2">
                <CardTitle className="flex justify-between items-center text-sm font-bold text-slate-700">
                    <span className="flex items-center gap-2"><PieIcon size={16} className="text-purple-600" /> Répartition</span>
                    <div className="flex bg-slate-100 rounded-lg p-0.5">
                        <button onClick={() => setMode('CAT')} className={`px-2 py-0.5 text-[10px] font-bold rounded-md transition-all ${mode === 'CAT' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400'}`}>Cat</button>
                        <button onClick={() => setMode('BEN')} className={`px-2 py-0.5 text-[10px] font-bold rounded-md transition-all ${mode === 'BEN' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400'}`}>Qui</button>
                    </div>
                </CardTitle>
            </CardHeader>
                            ...
                            <div key={idx} className="flex justify-between items-center text-xs">
                                <div className="flex items-center gap-1.5 min-w-0">
                                    ...
                                    <div className="flex flex-col">
                                        <span className="text-slate-700 font-medium truncate max-w-[80px]" title={item.name}>{item.name}</span>
