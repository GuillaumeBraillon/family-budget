...
export const VariableTransactionForm: React.FC<VariableTransactionFormProps> = ({ 
  ...
  const [accountId, setAccountId] = useState(accounts.filter(a => a.type === AccountType.CHECKING)[0]?.id || '');
  ...
  useEffect(() => {
    if (isOpen) {
        if (editingTransaction) {
            ...
            setDate(defaultDate); setLabel(''); setAmount(''); setCategory(''); setSubCategory('');
            setBeneficiaryId(defaultBeneficiary); setType('EXPENSE'); setIsWaiting(true); setIsExtra(false); setComments('');
        }
    }
  ...
  const handleSubmit = () => {
    ...
    onAddTransaction({
      ...
      date, label, amount: parseFloat(amount), category, subCategory, accountId, beneficiaryId, type, isWaiting, isExtra, comments: comments.trim() || undefined
    });
    onClose();
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={editingTransaction ? "Modifier l'opération" : "Nouvelle opération"}>
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-2">
            <div>
                ...
                <div className="flex bg-slate-100 p-1 rounded-lg">
                    <button type="button" onClick={() => setType('EXPENSE')} className={`flex-1 py-2 rounded-md text-sm font-bold transition-all ${isExpense ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500'}`}><TrendingDown size={14}/> Dépense</button>
                    <button type="button" onClick={() => setType('INCOME')} className={`flex-1 py-2 rounded-md text-sm font-bold transition-all ${!isExpense ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-500'}`}><TrendingUp size={14}/> Revenu</button>
                </div>
            </div>
            <div>
                <label className="text-xs font-medium text-slate-500 uppercase block mb-1">Pointage</label>
                <div className="flex bg-slate-100 p-1 rounded-lg">
                    <button type="button" onClick={() => setIsWaiting(false)} className={`flex-1 py-2 rounded-md text-sm font-bold transition-all ${!isWaiting ? 'bg-emerald-500 text-white' : 'text-slate-500'}`}><CheckCircle2 size={14}/> Réel</button>
                    <button type="button" onClick={() => setIsWaiting(true)} className={`flex-1 py-2 rounded-md text-sm font-bold transition-all ${isWaiting ? 'bg-amber-500 text-white' : 'text-slate-500'}`}><Clock size={14}/> Attente</button>
                </div>
            </div>
        </div>
        <SearchableTextInput label="Libellé" value={label} onChange={e => setLabel(e.target.value)} onSelectSuggestion={setLabel} suggestions={isExpense ? labelsSuggestions : []} required />
        ...
        <div className={`p-3 rounded-xl border-2 ${isExtra ? 'bg-amber-50 border-amber-300' : 'bg-slate-50 border-slate-200'}`}>
        ...
        <AccountSelector label="Compte" accounts={accounts.filter(a => a.type === AccountType.CHECKING)} value={accountId} onChange={e => setAccountId(e.target.value)} color={themeColor} />
        ...
        <TextInput label="Commentaire" value={comments} onChange={e => setComments(e.target.value)} icon={MessageSquare} />
        <div className="flex gap-3 pt-4 border-t border-slate-100">
            {editingTransaction && <button type="button" onClick={() => setShowDeleteConfirm(true)} className="px-4 py-3 bg-red-50 text-red-600 rounded-xl font-bold hover:bg-red-100"><Trash2 size={18} /></button>}
            <button onClick={handleSubmit} className={`flex-1 text-white py-3 rounded-xl font-bold transition-colors ${isExpense ? 'bg-indigo-600' : 'bg-emerald-600'}`}><Save size={18}/> Enregistrer</button>
        </div>
      </div>
    </Modal>
  );
