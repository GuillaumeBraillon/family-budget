...
export const MonthNavigator: React.FC<MonthNavigatorProps> = ({ date, onPrev, onNext }) => {
  ...
  return (
    <div className="flex items-center gap-4 bg-white p-2 rounded-xl border border-slate-200 shadow-sm">
      <button onClick={onPrev} className="p-2 hover:bg-slate-100 rounded-lg text-slate-600 transition-colors" aria-label="Mois précédent">
        <ChevronLeft size={20} />
      </button>
      ...
      <button onClick={onNext} className="p-2 hover:bg-slate-100 rounded-lg text-slate-600 transition-colors" aria-label="Mois suivant">
