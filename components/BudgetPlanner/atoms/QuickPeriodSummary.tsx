...
interface SummaryStats {
  ...
  planned: number;
  pending: number;
  extra: number; // Nouveau champ pour identifier les extras
}
    ...
    <div className="bg-gradient-to-r from-slate-900 to-indigo-900 text-white rounded-2xl p-5 shadow-xl flex flex-col md:flex-row items-stretch justify-between gap-6 animate-in slide-in-from-top-2 duration-500 border border-white/10">
      ...
      <div className="flex-1 flex flex-col justify-between">
        ...
        <div className="flex items-baseline gap-2">
            ...
            {expenses.extra > 0 && (
                <div className="flex items-center gap-1 text-[10px] bg-rose-500/20 text-rose-300 px-1.5 py-0.5 rounded font-bold border border-rose-500/30">
                    <Star size={10} /> {expenses.extra.toFixed(0)}€ Hors budget
                </div>
            )}
        </div>
        <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1">
            <MetricLabel icon={<CheckCircle2 size={10}/>} label="Réel" value={expenses.real} color="text-emerald-400" />
      ...
      <div className="flex-1 flex flex-col justify-between">
        ...
        <div className="flex items-baseline gap-2">
            ...
            {income.extra > 0 && (
                <div className="flex items-center gap-1 text-[10px] bg-emerald-500/20 text-emerald-300 px-1.5 py-0.5 rounded font-bold border border-emerald-500/30">
                    <Star size={10} /> {income.extra.toFixed(0)}€ Extra
                </div>
            )}
        </div>
        <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1">
            <MetricLabel icon={<CheckCircle2 size={10}/>} label="Réel" value={income.real} color="text-emerald-400" />
