-- =====================================================
-- MIGRATION 008 : Persistance paid_items.is_salary
-- =====================================================
-- Objectif : alimenter explicitement paid_items.is_salary
-- pour les instances issues de income_configs.is_salary = true.

BEGIN;

-- 1) Garantir la colonne
ALTER TABLE public.paid_items
ADD COLUMN IF NOT EXISTS is_salary boolean NOT NULL DEFAULT false;

-- 2) Backfill des opérations déjà pointées
-- Mapping via instance_id = {income_config_id}-YYYY-MM (ops récurrentes)
UPDATE public.paid_items pi
SET is_salary = true
FROM public.income_configs ic
WHERE pi.is_variable = false
  AND pi.type = 'INCOME'
  AND ic.is_salary = true
  AND pi.instance_id LIKE ic.id || '-%';

-- 3) Révoquer la signature RPC actuelle (avec p_is_refund, sans p_is_salary)
REVOKE EXECUTE ON FUNCTION public.upsert_paid_item_with_tags(
  text, numeric, date, text, text, text, text,
  public.transaction_type, boolean, boolean, boolean, boolean, text, jsonb, jsonb
) FROM authenticated;

-- 4) Recréer la RPC avec p_is_salary
